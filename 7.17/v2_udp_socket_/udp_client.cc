...
void *recv_from_server(void *args)
{
    ...
    while (1)
    {
        ...
        int n = recvfrom(sock, buffer, sizeof(buffer), 0, (sockaddr*)&temp, &length);
        if(n > 0)
        {
            buffer[n] = 0;
            cout << buffer << endl;
...
int main(int argc, char **args)
{
    ...
    // 创建一个线程, 负责收取从服务器端发来的信息, 然后标准输出
    pthread_t tid;
    pthread_create(&tid, nullptr, recv_from_server, &sock);
