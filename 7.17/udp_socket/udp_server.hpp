...
namespace Fjl
{
    ...
    {
    public:
        udp_server(handle handle_func, uint16_t port = 8080)
            : _handle_func(handle_func), _port_(port)
        {
        }
        ...
        void server_start()
        {
            ...
            while(1)
            {
                ...
                //处理后发送回客户端
                std::string str = _handle_func(buffer);

                //再向客户端发送回信息
                int ret = sendto(_socket_, str.c_str(), str.size(), 0, (sockaddr*)&temp, sizeof(temp));
            }
        }
