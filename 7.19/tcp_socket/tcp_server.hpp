    ...
    public:
        ...
        void server_init()
        {
            ...
            if (bind(_listen_sock, (sockaddr *)&local, sizeof(local)) < 0)
            {
                ...
                logMessage(Fatal, "socket bind error, code:%d, error string:%s", errno, strerror(errno));
                exit(BIND_ERROR);
            }

            logMessage(Fatal, "socket bind success");

            // 3.监听
            if (listen(_listen_sock, backlog))
            {
                // std::cout << "socket listen error: " << strerror(errno) << std::endl;
                logMessage(Fatal, "socket listen error, code:%d, error string:%s", errno, strerror(errno));
                exit(LISTEN_ERROR);
            }
            // std::cout << "all success" << std::endl;
            logMessage(Debug, "all success");
        }
        ...
        void server_start()
        {
            ...
            while (1)
            {
                ...
                if (sock < 0)
                {
                    ...
                    logMessage(Fatal, "accept error, code:%d, error string:%s", errno, strerror(errno));
                    continue;
                }
                ...
                // std::cout << "获取新连接成功: " << sock << "from" << _listen_sock << ","<< client_ip << ": " << client_port << std::endl;
                logMessage(Info, "获取新连接成功! %d from %d, client: %s-%d", sock, _listen_sock, client_ip.c_str(), client_port);
        ...
        void service(int sock, const std::string client_ip, const uint16_t client_port)
        {
            ...
            while (1)
            {
                ...
                if (n > 0)
                {
                    ...
                    logMessage(Info, "%s : %s", client.c_str(), res.c_str());
                ...
                else if (n == 0)
                {
                    ...
                    logMessage(Info, "%s quit", client.c_str());
                    break;
                }
                else
                {
